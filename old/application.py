...
def index():
    ...
    displayname = db.execute("SELECT displayname FROM users WHERE id IS :iden",iden=session["user_id"])
    ...
    return render_template("index.html",user=user[0]["username"],room=room,curround="null",players=returndata)
